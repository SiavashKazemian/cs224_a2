...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    self.input_placeholder = tf.placeholder(tf.float32,(self.config.batch_size,self.config.n_features),'mod_in')
    self.labels_placeholder = tf.placeholder(tf.int32,(self.config.batch_size,self.config.n_classes),'mod_out')
    ### END YOUR CODE
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope("softmax_layer"):
        W = tf.get_variable("weights", (self.config.n_features,self.config.n_classes), initializer=tf.constant_initializer(0.0))
        b = tf.get_variable("biases", (1,self.config.n_classes), initializer=tf.constant_initializer(0.0))
  ...
  def add_loss_op(self, pred):
    ...
    ### YOUR CODE HERE
    with tf.variable_scope("softmax_layer"):
        loss = cross_entropy_loss(self.labels_placeholder,self.add_model(self.input_placeholder))
    ### END YOUR CODE
    return loss
