...
class NERModel(LanguageModel):
  ...
  def create_feed_dict(self, input_batch, dropout, label_batch=None):
    ...
    Notes:
        borrowed code from: https://github.com/yihui-he/named-entity-recognition/blob/master/q2_NER.py
    """
  ...
  def add_embedding(self):
    ...
    Notes:
        great explanation of what tf.nn.embedding_loopup in:
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("layer"):
        W = tf.get_variable("W",(self.config.window_size*self.config.embed_size, self.config.hidden_size),initializer=xavier_initializer)
        b1 = tf.get_variable("b",(1,self.config.hidden_size),initializer=xavier_initializer)
        h = tf.tanh(tf.matmul(window,W)+b1)
        if self.config.l2:
            tf.add_to_collection('total_loss',0.5 * self.config.l2 * tf.nn.l2_loss(W))
    with tf.variable_scope("softmax"):
        U = tf.get_variable("U",(self.config.hidden_size, self.config.label_size),initializer=xavier_initializer)
        b2 = tf.get_variable("b",(1,self.config.label_size),initializer=xavier_initializer)
        y = tf.matmul(h,U)+b2
        if self.config.l2:
